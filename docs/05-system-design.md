...
- 幅520px以下では左サイドバーを画面下部の固定ナビゲーションへ変更し、主要な入力欄・ボタンを44px以上にする。
- 勤務表の日次行はCSSで日付単位のカードへ変換し、始業、終業、休憩、休暇、業務内容、システムNo.を横スクロールなしで編集可能にする。PC表示と印刷時はExcel型の表構造を維持する。
- Dockerなしデモは通常 `127.0.0.1` で待ち受ける。スマホ確認起動を明示的に選んだ場合だけ `0.0.0.0` へ切り替え、同一LANのIPv4アドレスを案内する。
